...
class EditEntryPage extends Component {
    ...
    handleSubmit(event) {
        ...
        //If they changed the state by more than just the next state
        const prevIdx = this.statuses.indexOf(this.props.state);
        const newIdx = this.statuses.indexOf(this.state.state);

        if (newIdx !== prevIdx) { // && this.state.state !== this.statuses[this.statuses.indexOf(this.props.state) + 1]) {
            //If state was changed to an earlier state
            if (newIdx < prevIdx) {

                for (let i = prevIdx; i > newIdx; i--) {
                    delete data[this.statuses[i] + "Timestamp"];
                }

            } else if (newIdx > prevIdx + 1) { //If state was changed to skip some states

                for(let i = prevIdx + 1; i <= newIdx; i++) {
                    data[this.statuses[i] + "Timestamp"] = currTime.valueOf();
                }

            } //Else the user selected the next step - so do nothing
                                ...
                                <Col md="12">
                                    ...
                                    <InputGroup className="input-group margin-bottom-sm">
                                        ...
                                        <TypeAheadInput name="classification"
                                                        value={this.humanReadableStatues[this.statuses.indexOf(this.props.data.state)]}
                                                        handleChange={this.handleInputChange} clearable={false}
                                                        getOptionsFromApi={false} creatable={false}
                                                        initialOptions={this.humanReadableStatues}/>
                                    </InputGroup>
                                </Col>
                            </Row>
                            <Row>
                                ...
                                <Col md="3">
                                    ...
                                    <InputGroup className="input-group margin-bottom-sm">
                                        ...
                                        <TypeAheadInput name="originator" value={this.state.originator}
                                                        handleChange={this.handleInputChange}
                                                        getOptionsFromApi={true} disabled={true}
                                                        initialOptions={[]}/>
                                    </InputGroup>
                                </Col>
                                <Col md="3">
